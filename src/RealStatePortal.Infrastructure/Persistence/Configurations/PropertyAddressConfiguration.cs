...
public sealed class PropertyAddressConfiguration : IEntityTypeConfiguration<PropertyAddress>
{
    public void Configure(EntityTypeBuilder<PropertyAddress> builder)
    {
        ...
        builder.HasIndex(address => new { address.City, address.PostalCode });

        builder.OwnsOne(address => address.Coordinates, coordinates =>
        {
            coordinates.Property(value => value.Latitude).HasColumnName("Latitude").HasPrecision(9, 6).IsRequired();
            coordinates.Property(value => value.Longitude).HasColumnName("Longitude").HasPrecision(9, 6).IsRequired();
        });
    }
}